...
@Component
@Path("/brandPersonal")
public class FPersonalService {
    @Bean
    public FPersonalService fPersonalService() {
        return new FPersonalService();
    }
    SmsVerifyKit s;
    private final static Logger logger = LoggerFactory.getLogger(FPersonalService.class);
    @Resource
    ...
    @Path("/{device}/brandPersonalRegist")
    public Response brandPersonalRegist(
        ...
        try {
            ...
            params.putAll(FMapBean.getPersonalService(fBrandBean));
            ...
            listPic.add(fBrandBean.getIdCardBack());
            listPic.add(fBrandBean.getIdCardFront());
            listPic.add(fBrandBean.getIdCardInHand());
            // 访问图片 查看图片是否真实存在
            ...
            content = HttpRequest.sendPost(Constant.BRAND_PERSONAL_REGIST, params);
            // 解析json
