...
public class ImageUtils {
     ...
     * @date 2017/12/30
     */
    public static ResultStruct accessImage(ResultStruct resultStruct, List<String> imagePath) throws IOException {
        for (int i = 0; i < imagePath.size(); i++) {
            Integer code = ImageUtils.checkInamge(imagePath.get(i));
            if (code != 200) {
                resultStruct.setStatus(ResultStruct.ERROR);
                resultStruct.setMsg(me.getValue(ResultMsgConstant.picError));
                return resultStruct;
            }
        }
        return resultStruct;
